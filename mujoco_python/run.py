...
def on_press(key):
    global goal_vel, goal_yaw_vel, goal_yaw
    ...
    try:
        if key.char == "o":
            ...
            goal_yaw = 0.0
        # 限幅保护：防止一直按住导致速度飙到天上
        goal_vel = max(-10.0, min(10.0, goal_vel))
        goal_yaw = max(-10.0, min(10.0, goal_yaw))
...
def ui_show():
    while True:
        display_text = (
            ...
            f"vel_error:{vel_error:.4f}\n"
            f"yaw:{math.degrees(yaw):.4f}\n"
            f"yaw_vel:{gyro[2]:.4f}\n"
            f"goal_yaw:{goal_yaw:.4f}\n"
        )
        UI.add_text(pos+[-0.1, -0.1, 0.4], display_text, color=[0, 1, 0, 1])
...
def main_loop():
    global target_wheel_vel, roll_error, vel_error, pos_error, yaw_error, yaw_vel_error, quat, gyro, acc, vel, pos, roll, pitch, yaw
    ...
    pos_error = pos_xz
    vel_error = goal_vel - vel[0]
    yaw_error = goal_yaw - yaw
    # yaw_vel_error = goal_yaw_vel - gyro[2]

    wheel_vel = (kp_roll * roll_error - kd_roll * gyro[0]) - (kp_pos * (pos_error) - kd_pos * vel[0]) -\
                (kp_vel * vel_error)
    
    # wheel_angvel = kp_yaw_vel * yaw_vel_error + kd_yaw_vel * yaw_vel_error
    wheel_yaw = kp_yaw * yaw_error + kd_vel * yaw_error
    
    target_wheel_vel = [wheel_vel - wheel_yaw/2.0, wheel_vel + wheel_yaw/2.0]
